...
class plot(fbasic):
    ...
    def points(x, y, size=None, color=None, marker=None):
        ...
        assert type(x) is list or type(
            x) is tuple, "type {} is not list or tuple".format(type(x))
        assert type(y) is list or type(
            y) is tuple, "type {} is not list or tuple".format(type(y))
        assert len(x) == len(
            y), "x length : {} not equal to y length : {}".format(
                len(x), len(y))
        points = []
